...
- **2025-12-17** – Mise à jour du `README_TECHNIQUE_FR.md` pour inclure le module `schemas` en plus de `conformity`, `collection`, `normalization` et `orchestration`.  
...
- Les modules doivent être exécutés en séquence :  
  ...
  5. **Schemas** (`base_schema.py`, `finance_schema.py`, `audit_schema.py`, `compliance_schema.py`) pour garantir la cohérence des données.  
...
La bitácora `core/architecture` trace désormais l’évolution complète du sous-module et de ses modules associés (`conformity`, `collection`, `normalization`, `orchestration`, `schemas`).  
